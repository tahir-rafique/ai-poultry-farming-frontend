...
function PredictContent() {
  ...
  const [form, setForm] = useState<PredictionInput>({ ...DEFAULTS, farm_id: farmId });
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="page-title flex items-center gap-2">
          <BrainCircuit size={24} className="text-primary-600" /> AI Profit Prediction
        </h1>
        ...
        <form onSubmit={handleSubmit} className="card p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="No. of Chicks">
              <input className="input" type="number" min={1} value={form.num_chicks} onChange={(e) => set("num_chicks", +e.target.value)} />
            </FormField>
            <FormField label="Chick Price (PKR/unit)">
              <input className="input" type="number" min={1} step="0.01" value={form.chick_price_per_unit} onChange={(e) => set("chick_price_per_unit", +e.target.value)} />
            </FormField>
            <FormField label="Feed Cost (PKR/kg)">
              <input className="input" type="number" min={1} step="0.01" value={form.feed_cost_per_kg} onChange={(e) => set("feed_cost_per_kg", +e.target.value)} />
            </FormField>
            ...
            <FormField label="Medicine Cost (PKR total)">
              <input className="input" type="number" min={0} step="1" value={form.medicine_cost_total} onChange={(e) => set("medicine_cost_total", +e.target.value)} />
            </FormField>
            <FormField label="Labor/Day (PKR)">
              <input className="input" type="number" min={0} step="1" value={form.labor_cost_per_day} onChange={(e) => set("labor_cost_per_day", +e.target.value)} />
            </FormField>
            <FormField label="Utilities Total (PKR)">
              <input className="input" type="number" min={0} step="1" value={form.utilities_cost_total} onChange={(e) => set("utilities_cost_total", +e.target.value)} />
            </FormField>
            ...
            <FormField label="Sell Price (PKR/kg)">
              <input className="input" type="number" min={1} step="0.01" value={form.expected_selling_price_per_kg} onChange={(e) => set("expected_selling_price_per_kg", +e.target.value)} />
            </FormField>
            ...
            <FormField label="Avg Live Weight (kg)">
              <input className="input" type="number" min={0.5} max={5} step="0.1" value={form.expected_avg_weight_kg} onChange={(e) => set("expected_avg_weight_kg", +e.target.value)} />
            </FormField>
          </div>
          ...
          {result && (
            <>
              ...
              <div className={`card p-5 border-2 ${
                result.verdict === "Profitable" ? "border-green-400 bg-green-50" :
                result.verdict === "Loss" ? "border-red-400 bg-red-50" :
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-2xl font-bold ${
                      result.verdict === "Profitable" ? "text-green-700" :
                      result.verdict === "Loss" ? "text-red-700" : "text-yellow-700"
                    }`}>{result.verdict}</p>
                  </div>
                  <div className="ml-auto text-right">
                    <p className="text-xs text-gray-400">Rule-based Profit</p>
                    <p className={`text-xl font-bold ${result.predicted_profit >= 0 ? "text-green-700" : "text-red-700"}`}>
                      {formatPKR(result.predicted_profit)}
                    </p>
                    <p className="text-xs text-gray-400 mt-1">ML Model: {formatPKR(result.ml_predicted_profit)}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <MetricCard icon={<DollarSign size={16} className="text-blue-600" />} label="Total Cost" value={formatPKR(result.cost_breakdown.total_cost)} bg="bg-blue-50" />
                <MetricCard icon={<BarChart2 size={16} className="text-green-600" />} label="Revenue" value={formatPKR(result.total_revenue)} bg="bg-green-50" />
                <MetricCard icon={<Percent size={16} className="text-purple-600" />} label="ROI" value={`${result.roi_percent}%`} bg="bg-purple-50" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <MetricCard icon={<BarChart2 size={16} className="text-orange-600" />} label="Profit Margin" value={`${result.profit_margin_percent}%`} bg="bg-orange-50" />
                <MetricCard icon={<BrainCircuit size={16} className="text-primary-600" />} label="Birds Sold" value={formatNumber(result.birds_sold)} bg="bg-primary-50" />
              </div>
